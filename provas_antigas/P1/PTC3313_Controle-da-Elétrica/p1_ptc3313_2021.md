...
## Questão 1 **(Valor: 1,5)**
Um engenheiro realizou um ensaio e obteve a seguinte função de transferência de um determinado processo
...
Solução final no domínio do tempo:
...
Calcule a FTMF entre $G_1(s) = \dfrac{1}{s(s+9)}$ e $H_1(s) = s$:

$$
\text{FTMF}_1 = \dfrac{G_1(s)}{1 +G_1(s)H_1(s)}
$$

$$
\text{FTMF}_1 = \dfrac{\dfrac{1}{s(s+9)}}{1 +\dfrac{1}{s(s+9)}\cdot s}
$$

$$
\text{FTMF}_1 = \dfrac{1}{s(s+10)}
$$

A função $G_2(s) = K\left(1+\dfrac{1}{s}\right)$ está em série com $\text{FTMF}_1$ no ramo direto formando um bloco $G_3(s)$:

$$
G_3(s) = G_2(s)\text{FTMF}_1
$$

$$
G_3(s) = K\left(1+\dfrac{1}{s}\right)\dfrac{1}{s(s+10)}
$$

$$
G_3(s) = K\dfrac{(s+1)}{s^2}\cdot \dfrac{1}{(s+10)}
$$

$$
G_3(s) = \dfrac{K(s+1)}{s^2(s+10)}
$$

Esta função está em malha fechada de realimentação unitária formando a relação desejada no enunciado. Portanto a FTMF é:

$$
\text{FTMF} = \dfrac{C(s)}{R(s)} = \dfrac{K(s+1)}{s^2(s+10)+K(s+1)}
$$

Ou ainda:

$$
\dfrac{C(s)}{R(s)} = \dfrac{K(s+1)}{s^3+10s^2+Ks+K}
$$

### b) Valores de $K$ para malha fechada estável

...

### c) Erro estacionário em rampa unitária

...

### d) Resposta de sistema 2ª ordem a degrau unitário
...
Usando o polinômio do denominador da
Aplicando a tabela de Routh para o polinômio do denominador da MF ($A(s) = 1s^3 + 6s^2 + 5s + K_c$), temos:

$$
...
$$

Para termos estabilidade, pelo critério de Routh, não deve haver trocas de sinal nos elementos da 1ª coluna de cima para baixo:
...
### c) Verificação de pontos no LGR e ganho correspondente
